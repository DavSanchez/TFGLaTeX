...
Tras editar estos ficheros, solo queda detener temporalmente la ejecución de FreeRADIUS, activar la autenticación SQL, comprobar que la configuración es correcta y volver a iniciar el servicio: \emph{service freeradius stop}.
...
En este punto comienza la compilación del código fuente. Al finalizar, tendríamos un fichero de nombre similar a \emph{coova-chilli\_1.3.0\_armhf.deb} en el directorio \emph{/usr/src/} y podríamos proceder a instalarlo situándonos en dicho directorio y utilizando el gestor de paquetes \emph{dpkg}.
...
\end{center}
\caption{Vista de la página de RADIUS Session-timeout de daloRADIUS}
% \source{http://coova.github.io/CoovaChilli/}
...
\end{center}
\caption{Estructura del directorio en el que se incluye el código programado para el módulo Usuario}
% \source{http://coova.github.io/CoovaChilli/}
\label{repoGitHub}
\end{figure}

%A excepción de los directorios scripts y csvgen y del fichero CaptivePortal.iml, correspondientes respectivamente a utilidades de configuración y ficheros propios del \emph{software} de trabajo, c
...
Estos ficheros controlan la base de datos detallada en el apartado anterior, consultando y actualizando sus valores a petición del servidor. Sus funciones están definidas mediante los siguientes métodos y requerimientos.
...
El objeto \emph{fs} es la importación del módulo \emph{fs} de Node.js, que permite la lectura y escritura síncrona o asíncrona de ficheros. Lo utilizamos para actualizar los campos de la base de datos según sea necesario. El objeto \emph{userObj}, como puede observarse, es la importación del fichero de la propia base de datos JSON.
...
La siguiente ruta a \emph{/creds} se encarga de pasar a la \emph{Aplicación Web} el valor actual de la variable \emph{creds} implementada al principio del código fuente del servidor. El valor de dicho objeto cambiará, obteniendo las credenciales de un usuario recién marcado como activo del controlador de usuarios, cuando un usuario que no esté conectado al servicio envíe un fichero de audio por primera vez al servidor.
...
A continuación se detalla la función \emph{setCreds()}, que será de vital importancia para los siguientes manejadores de ruta.
...
Esta función tiene de cometido modificar los valores del objeto \emph{creds}, que pasan a contener las credenciales del usuario enviado desde el controlador de usuarios, almacenadas previamente en la variable local \emph{data}. Existe además una función similar a esta llamada \emph{setCredsOneTime()} que se encarga de la misma tarea pero utilizando el controlador correspondiente a los usuarios con una conexión única de 30 minutos.

Una vez vistas las funciones \emph{setCreds()} y \emph{setCredsOneTime()} se detallan los manejadores de ruta más relevantes del \emph{Servidor}, los que gestionan la llegada de ficheros de audio y los guardan en el directorio \emph{uploads} del sistema de ficheros.
...
Estos tres manejadores de ruta son prácticamente idénticos en cuanto a que gestionan la llegada de ficheros de audio al sistema, la única diferencia que existe entre ellos radica que la ruta \emph{/loggedupload} se utiliza cuando el fichero de audio proviene de una instancia de la \emph{Aplicación Web} cuyo usuario ya está conectado al servicio —esto es, tiene acceso a Internet—, por lo que no hace falta buscar unas credenciales inactivas nuevas para que le sean proporcionadas. Por ello, en el \emph{callback} asociado al evento de finalización de la transacción de ficheros, el primer manejador de ruta (\emph{/upload}) llama a la función \emph{setCreds()} para preparar las credenciales que son enviadas con las peticiones a las rutas vistas anteriormente. La ruta \emph{/onetimepassupload} se encarga de los ficheros de audio procedentes de los usuarios que han optado por la conexión de 30 minutos.

Nótese también la diferencia entre la ruta de la petición HTTP de la primera ruta (\emph{upload}) y la ruta del directorio de destino en el sistema de ficheros (\emph{uploads}), que puede llevar a conclusión debido a la similitud de los nombres.
...
Donde la variable \emph{options} contiene los ficheros de certificado, clave y la \emph{passphrase} necesarios para ofrecer el servicio.
...
También hay que habilitar el uso de SSL en el objeto \emph{chilliController}, definiendo el atributo necesario en el fichero \emph{chilliController.js}, que analizaremos más adelante, con la declaración: \emph{chilliController.ssl = true;}.
...
La función \emph{liberateUser()} se analiza en el apartado \ref{script.js} dedicado al fichero \emph{scripts.js}.
...
La funcionalidad de la Aplicación Web viene determinada en gran medida por este fichero JavaScript, que hace uso de los siguientes variables y métodos.
...
Por último, la \emph{Aplicación Web} tiene un campo de alertas que notifican al usuario de la recepción de ficheros de audio o errores durante las mismas mediante la función \emph{setAlert()}, llamada por \emph{errorHandler()} otras funciones de este fichero ya expuestas anteriormente. Las alertas siguen la estética del resto de la aplicación web, puede verse un ejemplo de alerta por fichero enviado en la Figura \ref{alertaBootsOK} y su mecanismo de funcionamiento en el bloque de código \ref{Alertas}.
