...
La última línea, \verb+post-up echo 1 > /proc/sys/net/ipv4/ip_forward+, es la activación del \emph{flag} del \emph{kernel} que habilita el paso de datagramas IP tratados en la descripción del módulo de Control de Red. Si se prefiere utilizar esta orden de otra forma y así asegurarse de que el \emph{flag} queda activado también puede editarse el archivo \verb+/etc/sysctl.conf+, quitando el carácter de comentario (\verb+#+) en la línea \verb+net.ipv4.ip_forward=1+ y reiniciando el servicio de red mediante la siguiente orden: \mintinline{bash}{/etc/init.d/networking restart}.

Una vez configurada la interfaz inalámbrica pueden instalarse los paquetes necesarios para preparar los módulos. Se incluyen algunos paquetes previos necesarios para la instalación posterior de \emph{software}, como herramientas para transferencia de datos (\emph{libcurl}), utilidades de red manejadas por CoovaChilli (\emph{iptables}), compiladores (\emph{gcc}) y gestores de dependencias para los mismos (\emph{make}). Recordamos que esta instalación debe ser realizada por el superusuario, por lo que se debe entrar en \emph{root} ejecutando primero la orden \mintinline{bash}{sudo su} o por el contrario utilizar la orden previa \mintinline{bash}{sudo} para cada orden a introducir. Además, pueden instalarse todos los paquetes necesarios con una sola orden escribiendo todos los nombres seguidos después de la orden \mintinline{bash}{apt-get install} y forzando una respuesta afirmativa a todas las preguntas que nos realice el terminal al instalar los paquetes usando los modificadores \mintinline{bash}{-y --force-yes}.
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{sudo apt-get install -y --force-yes debconf-utils}
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{echo 'mysql-server mysql-server/root_password password CONTRASEÑA' | debconf-set-selections}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{echo 'mysql-server mysql-server/root_password_again password CONTRASEÑA' | debconf-set-selections}

Tras esto, ya pueden instalarse el resto de paquetes necesarios que habíamos comentado al principio.

\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{sudo apt-get install -y --force-yes debhelper libssl-dev libcurl4-gnutls-dev mysql-server freeradius freeradius-mysql gcc make libnl1 libnl-dev pkg-config iptables}
...
Primero se crea la base de datos con la orden \verb+mysql+, utilizando el usuario root y la contraseña especificada anteriormente.

\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{echo 'create database radius' | mysql -u root -p CONTRASEÑA}
...
Como opción antes de ejecutar estas órdenes, y para incrementar la seguridad, se puede modificar la contraseña por defecto de FreeRADIUS para operar en la base de datos (\emph{radpass}) en el archivo \verb+/etc/freeradius/sql/mysql/admin.sql+.

El siguiente paso es habilitar el soporte de SQL en el archivo de configuración de FreeRADIUS y como opción activar los contadores diarios. Para ello se debe editar el archivo ubicado en \verb+/etc/freeradius/radiusd.conf+ y eliminando el carácter de comentario (\verb+#+) de las líneas \verb+$INCLUDE sql.conf+, \verb+$INCLUDE sql/mysql/counter.conf+ y opcionalmente de \verb+daily+.

Tras editar estos archivos, solo queda detener temporalmente la ejecución de FreeRADIUS, activar la autenticación SQL, comprobar que la configuración es correcta y volver a iniciar el servicio: \mintinline{bash}{service freeradius stop}.

Para activar la autenticación SQL hay que editar el archivo \verb+/etc/freeradius/sites-available/default+ y quitar los comentarios de todas las líneas que contengan \verb+sql+. Una vez hecho esto comprobamos la configuración y reanudamos el servicio.

\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{freeradius -C}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{service freeradius start}
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{apt-get install -y --force-yes git libjson-c-dev haserl gengetopt devscripts libtool bash-completion autoconf automake}
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{cd /usr/src}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{git clone https://github.com/coova/coova-chilli.git}
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{cd /usr/src/coova-chilli}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{dpkg-buildpackage -us -uc}

En este punto comienza la compilación del código fuente. Al finalizar, tendríamos un archivo de nombre similar a \verb+coova-chilli_1.3.0_armhf.deb+ en el directorio \verb+/usr/src/+ y podríamos proceder a instalarlo situándonos en dicho directorio y utilizando el gestor de paquetes \emph{dpkg}.

\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{cd /usr/src}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{dpkg -i coova-chilli_1.3.0_armhf.deb}

\paragraph{Configuración Inicial de CoovaChilli} \label{CoovaConfig} ~\\

Llegados a este punto el programa CoovaChilli está instalado y podemos proceder a su configuración. Esta configuración en su mayoría se basa, como se adelantó en la descripción del \emph{software} del anterior capítulo, en modificar el archivo \verb+/etc/chilli/defaults+ o hacer una copia de éste bajo el nombre \verb+config+ teniendo allí las modificaciones con respecto a \verb+defaults+. Sin embargo, hay dos pasos previos a la configuración de este archivo.

En primer lugar debemos modificar el \emph{script} que CoovaChilli ejecuta al ponerse en marcha la interfaz TUN (ubicado en \verb+/etc/chilli/up.sh+) introduciendo una regla de la herramienta \emph{iptables} para permitir el reenvío de los paquetes IP desde la interfaz de red cableada \emph{eth0}, que se almacena en una variable interna de la configuración de CoovaChilli llamada \verb+HS_WANIF+.

\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{iptables -I POSTROUTING -t nat -o $HS_WANIF -j MASQUERADE}

También debemos activar el \emph{flag} que permite activar CoovaChilli en el archivo \verb+/etc/default/chilli/+ cambiando el valor de la variable \verb+START_CHILLI+ de 0 a 1.

Tras esto, se debe editar el archivo de configuración de CoovaChilli introduciendo los valores adecuados a nuestro sistema en los atributos relevantes en este momento en el archivo \verb+/etc/chilli/defaults+ o bien en su copia renombrada como \verb+/etc/chilli/config+, que al iniciar el sistema reemplaza los valores dados en defaults donde difieran. Dado que en este momento aún no se encuentra instalada la herramienta \emph{hostapd} (ver más adelante) aún no tenemos habilitado un SSID para nuestro sistema, por lo que en un paso posterior tras la instalación de \emph{hostapd} debemos volver a este archivo de configuración e introducir el nombre del SSID que hayamos establecido. En el caso de este TFG, el SSID utilizado es \emph{RasPiDav}.
...
Debido al funcionamiento de nuestro portal cautivo, que opera con la interfaz JSON de CoovaChilli, es necesario dejar en blanco el campo \verb+HS_UAMSECRET+, de lo contrario la aplicación Web no podría iniciar sesión en el servicio con las credenciales obtenidas.

\paragraph{Instalación de Haserl} \label{HaserlInstall} ~\\
...
Tras esto actualizamos la ruta de \emph{Haserl} para CoovaChilli en el archivo \verb+/etc/chilli/wwwsh+ buscando la línea \verb+haserl=+ y modificándola a \verb+haserl=/usr/local/bin/haserl+. Tras esto, solo queda poner en marcha CoovaChilli con la siguiente orden.

\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{service chilli start}
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{apt-get install -y --force-yes hostapd}

Cuando termine la instalación se edita el archivo de configuración \verb+/etc/default/hostapd+ añadiendo la ruta del archivo de configuración que contiene los atributos deseados para nuestro punto de acceso, que está ubicado en \verb+/etc/hostapd/hostapd.conf+. Para ello buscamos la línea que contenga \verb+DAEMON_CONF=+, nos aseguraremos de que está sin comentar y le especificaremos la ruta entre comillas, de manera que quede de la siguiente forma: \verb+DAEMON_CONF="/etc/hostapd/hostapd.conf"+.

Ahora configuramos el mismo archivo cuya ruta hemos indicado en el ajuste anterior, indicando todos los atributos para nuestro punto de acceso incluyendo el driver a utilizar (\emph{nl80211}). Recordamos que la interfaz establecida ha de ser la misma que indicamos en el archivo de configuración \verb+defaults+ o \verb+config+ de CoovaChilli como \verb+HS_LANIF+, en nuestro caso \emph{wlan0}, que tiene la red estática ya configurada en el archivo interfaces al principio de la implantación. Además, el SSID también ha de ser el mismo que indicamos a CoovaChilli con la variable \verb+HS_SSID+.
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{service hostapd start}
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{apt-get install -y --force-yes php5-mysql php-pear php5-gd php-db php5-fpm libgd2-xpm-dev libpcrecpp0 libxpm4 nginx php5-xcache}

Cuando acabe esta instalación el programa \emph{NGINX} queda instalado y podríamos alojar el contenido Web que deseamos servir en el directorio \verb+/usr/share/nginx/html/+. En nuestro caso, descargamos los contenidos de daloRADIUS, los descomprimimos allí y renombramos el directorio generado para acceder a él al teclear la futura URL desde el navegador.
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{nginx -t}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{service nginx restart}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{service hostapd restart}

Tras esto podemos acceder a \emph{daloRADIUS} desde el navegador para realizar las gestiones deseadas utilizando la IP proporcionada a la interfaz cableada. Por ejemplo, si la IP de dicha interfaz es 192.168.1.30 se puede acceder a \emph{daloRADIUS} accediendo a la URL \verb+http://192.168.1.30/daloradius/+.
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{curl -sL https://deb.nodesource.com/setup_8.x | sudo -E bash -}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{sudo apt-get install -y nodejs}
...
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{openssl req -x509 -sha256 -newkey rsa:4096 -keyout key.pem -out cert.pem -days 365}
...
También hay que habilitar el uso de SSL en el objeto \emph{chilliController}, definiendo el atributo necesario en el archivo \emph{chilliController.js}, que analizaremos más adelante, con la declaración: \mintinline{javascript}{chilliController.ssl = true;}.

Por último, el propio \emph{software} CoovaChilli debe tener habilitado SSL en su configuración y referenciar el certificado y clave en la ruta donde se encuentren. Para ello se edita el archivo \verb+/etc/chilli/config+ tal y como se vio en apartados anteriores, definiendo los siguientes atributos.

\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{HS_REDIRSSL=on}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{HS_SSLKEYFILE=/etc/chilli/sslkey/cert.pem}
\mint[breaklines, breakanywhere, breaksymbolleft={}]{bash}{HS_SSLCERTFILE=/etc/chilli/sslkey/cert.pem}
...
La función \emph{liberateUser()} se analiza en el apartado \label{script.js} dedicado al archivo \emph{scripts.js}.
